...
If your cache update logic would be too complicated to maintain, it can be more pragmatic to simply refetch data from the server. Unfortunately, this can easily result in a barrage of requests, depending on how many queries are affected. To prevent this, you can use the `lazyRefetch` option, which allows you to specify which fields on the `ROOT_QUERY` should be refreshed. For each currently active query that contains any of the specified fields at the top level of its selection set (aliased or not), a minimal query is sent containing just the matching fields rather than the entire query - the responses update the cache and thereby all components consuming the fields. Cached occurrences of the fields that no active query contains are evicted, so that they are refetched (again minimally, thanks to query reduction) the next time a query using them becomes active. This serves to keep both the number and the size of the requests to a minimum while still ensuring that your UI is up-to-date.

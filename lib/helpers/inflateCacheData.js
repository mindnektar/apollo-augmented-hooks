...
var hasDuplicateTypenameSubPaths = function hasDuplicateTypenameSubPaths(typenamePath, typename) {
  var subPaths = typenamePath.reduce(function (result, key, index) {
    if (typenamePath[index - 1] === typename) {
      result.push('');
    }

    return [].concat((0, _toConsumableArray2["default"])(result.slice(0, -1)), ["".concat(result[result.length - 1] || '', ".").concat(key)]);
  }, ['']);
  subPaths[subPaths.length - 1] = "".concat(subPaths[subPaths.length - 1], ".").concat(typename);
  var lastSubPath = subPaths.pop();
  return subPaths.includes(lastSubPath);
};

var maybeInflate = function maybeInflate(cache, cacheContents, aliases, item, typenamePath) {
  ...
  if (!cacheKey) {
    return inflate(cache, cacheContents, aliases, cacheItem, typenamePath);
  ...
  if (!itemCache[cacheKey]) {
    ...
    var inflatedItem = inflate(cache, cacheContents, aliases, cacheItemWithAliases, [].concat((0, _toConsumableArray2["default"])(typenamePath), [cacheItem.__typename]));

    if (!typenamePath.includes(cacheItem.__typename)) {
      itemCache[cacheKey] = inflatedItem;
    }
  ...
  return Object.entries(data).reduce(function (result, _ref3) {
    ...
    if (Array.isArray(item)) {
      return _objectSpread(_objectSpread({}, result), {}, (0, _defineProperty2["default"])({}, fieldName, item.reduce(function (itemResult, entry) {
        var inflatedEntry = maybeInflate(cache, cacheContents, aliases, entry, typenamePath);
    ...
    if ((0, _typeof2["default"])(item) === 'object') {
      return _objectSpread(_objectSpread({}, result), {}, (0, _defineProperty2["default"])({}, fieldName, maybeInflate(cache, cacheContents, aliases, item, typenamePath)));
    }

...
var maybeInflate = function maybeInflate(cache, cacheContents, item, path) {
  ...
  var cacheItem = cacheContents[cache.identify(item)] || item;
  return !cacheItem.__typename || !path.includes(cacheItem.__typename) ? inflate(cache, cacheContents, cacheItem, cacheItem.__typename ? [].concat(_toConsumableArray(path), [cacheItem.__typename]) : path) : _objectSpread(_objectSpread({}, (0, _keyFields.keyFieldsForTypeName)(cache, cacheItem.__typename).reduce(function (result, keyField) {

...
var hasDuplicateTypenameSubPaths = function hasDuplicateTypenameSubPaths(typenamePath, cacheKey) {
  ...
  subPaths[subPaths.length - 1] = "".concat(subPaths[subPaths.length - 1], ".").concat(cacheKey);
  var lastSubPath = subPaths.pop();
  return subPaths.includes(lastSubPath);
};

var maybeInflate = function maybeInflate(cache, cacheContents, aliases, item, typenamePath, itemCache) {
  ...
  if (!itemCache[cacheKey]) {
    ...
    itemCache[cacheKey] = inflate(cache, cacheContents, aliases, cacheItemWithAliases, [].concat((0, _toConsumableArray2["default"])(typenamePath), [cacheItem.__typename]), itemCache);
  }
...
var inflate = function inflate(cache, cacheContents, aliases, data, typenamePath, itemCache) {
  return Object.entries(data).reduce(function (result, _ref3) {
    ...
    if (Array.isArray(item)) {
      return _objectSpread(_objectSpread({}, result), {}, (0, _defineProperty2["default"])({}, fieldName, item.reduce(function (itemResult, entry) {
        var inflatedEntry = maybeInflate(cache, cacheContents, aliases, entry, typenamePath, itemCache);
    ...
    if ((0, _typeof2["default"])(item) === 'object') {
      return _objectSpread(_objectSpread({}, result), {}, (0, _defineProperty2["default"])({}, fieldName, maybeInflate(cache, cacheContents, aliases, item, typenamePath, itemCache)));
    }

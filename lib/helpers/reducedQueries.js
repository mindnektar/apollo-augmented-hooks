...
var filterSubSelections = function filterSubSelections(selections, cacheData, cacheObjectsOrRefs, variables) {
  ...
  var reducedSelections = selections.reduce(function (result, selection) {
    ...
    if ( // Always keep the id field, otherwise apollo can't merge the cache items after the
    // request is done.
    fieldName === 'id' // Keep the entire selection if at least one of its items is not in the cache (it may
    ...
    if (selection.selectionSet) {
      ...
      // deeper-level fields.
      var nextCacheObjectsOrRefs = findNextCacheObjectsOrRefs(cacheData, cacheObjectsOrRefs, fieldName); // If we can't find any data for this field in the cache at all, we'll keep the entire
      ...
      return handleSubSelections(result, selection, cacheData, nextCacheObjectsOrRefs, variables);
    }
...
var handleSubSelections = function handleSubSelections(result, selection, cacheData, cacheObjectsOrRefs, variables) {
  var subSelections = filterSubSelections(selection.selectionSet.selections, cacheData, cacheObjectsOrRefs, variables);
...
var makeReducedQueryAst = function makeReducedQueryAst(cache, queryAst, variables) {
  ...
  var selections = queryAst.definitions[0].selectionSet.selections.reduce(function (result, selection) {
    ...
    return handleSubSelections(result, selection, cacheContents, cacheObjectsOrRefs, variables);

...
var buildFieldName = function buildFieldName(selection) {
  ...
  var variables = selection.arguments.reduce(function (result, _ref) {
    var name = _ref.name,
        value = _ref.value;
    return _objectSpread(_objectSpread({}, result), {}, _defineProperty({}, name.value, value.value));
  ...
  return "".concat(selection.name.value, "(").concat((0, _jsonStableStringify["default"])(variables), ")");
...
var filterSubSelections = function filterSubSelections(selections, cacheData, cacheObjectOrRef) {
  ...
  var reducedSelections = selections.reduce(function (result, selection) {
    var fieldName = buildFieldName(selection); // The current field is not a leaf in the tree, so we may need to go deeper.
    ...
    if (selection.selectionSet) {
      ...
      return handleSubSelections(result, selection, cacheData, cacheObject[fieldName]);
...
var handleSubSelections = function handleSubSelections(result, selection, cacheData, cacheObjectOrRef) {
  var subSelections = filterSubSelections(selection.selectionSet.selections, cacheData, cacheObjectOrRef);
...
var makeReducedQueryAst = function makeReducedQueryAst(cache, queryAst) {
  ...
  var selections = queryAst.definitions[0].selectionSet.selections.reduce(function (result, selection) {
    ...
    var fieldName = buildFieldName(selection);
    ...
    if (Array.isArray(cacheObjectOrRef)) {
      var _cacheObjectOrRef$;

      var hasRefs = !!((_cacheObjectOrRef$ = cacheObjectOrRef[0]) !== null && _cacheObjectOrRef$ !== void 0 && _cacheObjectOrRef$.__ref);

      if (hasRefs && cacheObjectOrRef.every(function (_ref2) {
        var __ref = _ref2.__ref;
        return cacheContents[__ref];
      })) {
        return handleSubSelections(result, selection, cacheContents, cacheObjectOrRef);
      }
    } else {
      var hasRef = !!(cacheObjectOrRef !== null && cacheObjectOrRef !== void 0 && cacheObjectOrRef.__ref);

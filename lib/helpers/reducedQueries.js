...
exports.makeReducedQueryAst = exports.clearReducedQueryCache = exports.hasVariable = void 0;
...
var reducedQueryCache = new _trie.Trie(); // cacheObjectOrRef may contain either the actual cache object or a reference to it. In the latter
...
var isPresentInCache = function isPresentInCache(cacheData, cacheObjectOrRef, fieldNames) {
...
var findNextCacheObjectsOrRefs = function findNextCacheObjectsOrRefs(cacheData, cacheObjectsOrRefs, fieldNames) {
  return cacheObjectsOrRefs.reduce(function (result, item) {
    ...
    var fieldName = fieldNames.find(function (value) {
      return itemCacheObject[value] !== undefined;
    });
...
var filterSubSelections = function filterSubSelections(selections, cacheData, cacheObjectsOrRefs, variables, keyFields) {
  ...
  var reducedSelections = selections.reduce(function (result, selection) {
    ...
    var fieldNames = (0, _fieldNames.buildFieldNames)(selection, variables);

    if ( // Always keep any key fields, otherwise apollo can't merge the cache items after the
    // request is done.
    isKeyField(cacheData, cacheObjectsOrRefs, selection.name.value, keyFields) // Keep the entire selection if at least one of its items is not in the cache (it may
    // have been evicted at some point).
    || !cacheObjectsOrRefs.every(function (item) {
      return isPresentInCache(cacheData, item, fieldNames);
    })) {
      return [].concat((0, _toConsumableArray2["default"])(result), [selection]);
    ...
    if (selection.selectionSet) {
      ...
      // deeper-level fields.
      var nextCacheObjectsOrRefs = findNextCacheObjectsOrRefs(cacheData, cacheObjectsOrRefs, fieldNames); // If we can't find any data for this field in the cache at all, we'll drop the entire
...
var clearReducedQueryCache = function clearReducedQueryCache() {
  reducedQueryCache = new _trie.Trie();
};

exports.clearReducedQueryCache = clearReducedQueryCache;

var makeReducedQueryAst = function makeReducedQueryAst(cache, queryAst, variables) {
  var cacheKey = [queryAst, (0, _jsonStableStringify["default"])(variables)];
  var ref = reducedQueryCache.lookupArray(cacheKey);
  ...
  var reducedQueryAst = _objectSpread(_objectSpread({}, queryAst), {}, {
    definitions: queryAst.definitions.map(function (definition) {
      ...
      var selections = definition.selectionSet.selections.reduce(function (result, selection) {
        ...
        var fieldNames = (0, _fieldNames.buildFieldNames)(selection, variables);
        var fieldName = fieldNames.find(function (item) {
          var _cacheContents$ROOT_Q;

          return ((_cacheContents$ROOT_Q = cacheContents.ROOT_QUERY) === null || _cacheContents$ROOT_Q === void 0 ? void 0 : _cacheContents$ROOT_Q[item]) !== undefined;
        });
        var cacheObjectsOrRefs = (_cacheContents$ROOT_Q2 = cacheContents.ROOT_QUERY) === null || _cacheContents$ROOT_Q2 === void 0 ? void 0 : _cacheContents$ROOT_Q2[fieldName];

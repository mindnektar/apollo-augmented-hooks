...
var shouldResetReducedQueries = false; // A helper that adds/removes a cache object to/from an array, depending on whether the handler
    ...
    var next = origin.filter(function (ref) {
      return subjects.some(function (subject) {
        return !keyFields.every(function (keyField) {
          return details.readField(keyField, ref) === details.readField(keyField, subject);
        });
      });
    });

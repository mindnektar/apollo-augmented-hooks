...
var _useForceUpdate3 = _interopRequireDefault(require("./useForceUpdate"));
...
var _default = function _default(useQueryHook, queryAst, options) {
  ...
  var _useForceUpdate = (0, _useForceUpdate3["default"])(),
      _useForceUpdate2 = (0, _slicedToArray2["default"])(_useForceUpdate, 2),
      forceUpdate = _useForceUpdate2[0],
      updateKey = _useForceUpdate2[1];

  var reducedQueryAst = (0, _react.useMemo)(function () {
    return getQueryAst(queryAst, client, options);
  }, [JSON.stringify(options.variables || {}), updateKey]);
  var skip = !reducedQueryAst || options === _client.skipToken || options.skip; // If all the requested data is already in the cache, we can skip this query.

  var queryOptions = skip && useQueryHook === _client.useSuspenseQuery ? _client.skipToken : _objectSpread(_objectSpread({}, options), {}, {
    context: _objectSpread(_objectSpread({}, globalContext), options.context),
    skip: skip && useQueryHook === _client.useQuery,
  ...
  var reducedResult = useQueryHook(reducedQueryAst || queryAst, queryOptions); // Remember all the query requests that are currently in flight, so we can ensure that any mutations
